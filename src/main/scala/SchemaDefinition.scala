  ...
  val CommentType =
    ObjectType(
      ...
      fields[ArticleRepository, Comment](
        ...
        Field("articleId", StringType, Some("コメントがついている記事のId"), resolve = _.value.articleId),
  ...
  val ArticleType =
    ObjectType(
      ...
      fields[ArticleRepository, Article](
        ...
        Field("id", StringType, Some("記事のId"), resolve = _.value.id),
        Field("title", StringType, Some("記事のタイトル"), resolve = _.value.title),
        Field("author", OptionType(StringType), Some("記事の著者"), resolve = _.value.author),
